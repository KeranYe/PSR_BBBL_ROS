...
void drive_Callback(const geometry_msgs::Twist::ConstPtr& cmd_vel_twist)
{
  ...
  running_time_desired   = cmd_vel_twist->linear.z;
...
{
  ros::init(argc, argv, "psr_control");

  ros::NodeHandle npub;

  ros::Publisher psr_pub = npub.advertise<geometry_msgs::Twist>("turtlebot_teleop/cmd_vel",1);

  ros::Rate rate(100);
  while(ros::ok()){
    geometry_msgs::Twist twist_msg;
    twist_msg.linear.x = linear_premap;
    twist_msg.angular.z = angular_premap;

    psr_pub.publish(twist_msg);
    ROS_INFO("Sending Twist Message: [linear.x = %f, angular.z = %f]", twist_msg.linear.x, twist_msg.angular.z);
